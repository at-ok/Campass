...
編集フォームは未保存の変更がある場合、閉じるボタン・キャンセル・Escape・背景クリックで破棄確認を表示する。再読み込みやタブを閉じる操作にはブラウザー標準の離脱確認を使用する。

一般予定の種類は「その他」「リマインダー」のみ。授業・課題・試験はそれぞれ専用の登録フォームで作成する。旧分類の一般予定はそのまま閲覧でき、編集フォームでは「その他」として扱い、保存時に分類を更新する。既存データの一括変更は行わない。
